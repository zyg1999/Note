...
TCP协议是**面向连接的，可靠的，基于字节流的**传输协议。在基于 TCP 进行通信时，通信双方需要先建立一个 TCP 连接，建立连接需要经过三次握手，断开连接的时候需要经过四次挥手。
...
对于 TCP 头部来说，以下几个字段是很重要的:
 ...
 - `确认号` （Acknowledgement Number），这个序号表示数据接收端期望接收的下一个字节的编号是多少，同时也表示上一个序号的数据已经收到。Ack = Seq+1
...
### 为什么 A 要进入 TIME-WAIT 状态，等待 2MSL 时间后才进入 CLOSED 状态？（MSL 最长报文段寿命 ）

**为了保证 B 能收到 A 的确认应答**。网络是不可靠的， 有可能最后一个ACK丢失 。若B在1MSL后没有收到A的Ack包， **服务器可以超时重传**这个FIN-ACK ，超时重传也需要1MSL。 接着客户端再重传一次确认，重新启动时间等待计时器。若 A 发完确认应答后直接进入 CLOSED 状态，则服务器有可能不会正常关闭。
...
而关闭连接时，服务器收到对方的FIN报文时，发一个ACK包仅仅表示对方不再发送数据了但是还能接收数据，而自己也未必全部数据都发送给对方了。当服务器发完了才发FIN包表示发送数据完毕。
